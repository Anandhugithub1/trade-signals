"""
Apply (or update) the nifty_option_signals table in Supabase.
...
SCHEMA_FILE = Path(__file__).resolve().parent.parent / "schema" / "nifty_option_signals.sql"
    ...
    try:
        ...
        with psycopg.connect(db_url) as conn:
            with conn.cursor() as cur:
                cur.execute(sql)
            conn.commit()
        print("[OK] nifty_option_signals schema applied / up to date.")
        return 0
